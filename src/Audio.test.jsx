...
test('Can render.', () => {
  render(<Audio url={url} isPlaying={false} doVisualization={false} filename={'foo.mp3'} onPlayPressed={id} onPausePressed={id}/>);
});

test('Clicking play triggers the callback.', () => {
  const mockCallback = jest.fn();
  const { getByRole } = render(
    <Audio url={url} isPlaying={false} doVisualization={false} filename={'foo.mp3'} onPlayPressed={mockCallback} onPausePressed={id}/>
  );
...
test('Clicking pause triggers the callback', () => {
  ...
  const { getByRole, queryByLabelText, getByLabelText } = render(
    <Audio url={url} isPlaying={true} doVisualization={false} filename={'foo.mp3'} onPlayPressed={id} onPausePressed={mockCallback}/>
  );
...
test('Audio can be played.', () => {
  const { getByRole, queryByLabelText } = render(
    <Audio url={url} isPlaying={true} doVisualization={false} filename={'foo.mp3'} onPlayPressed={id} onPausePressed={id}/>
  );
...
test('Calls onEnded when the audio is finished playing.', () => {
  const mockCallback = jest.fn();
  const { getByRole, queryByLabelText, getByLabelText } = render(
    <Audio url={url} isPlaying={true} doVisualization={false} filename={'foo.mp3'} onPlayPressed={id} onPausePressed={id} onEnded={mockCallback}/>
  );
